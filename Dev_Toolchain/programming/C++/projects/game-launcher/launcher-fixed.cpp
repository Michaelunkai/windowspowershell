    ...
    if (hFind != INVALID_HANDLE_VALUE) {
        do {
            ...
            if (lowerName.find(L"unins") == wstring::npos &&
                ...
                lowerName.find(L"directx") == wstring::npos &&
...
void ScanGames() {
    ...
    if (!DirectoryExists(gamesFolder)) {
        if (hListBox) {
            SendMessageW(hListBox, LB_RESETCONTENT, 0, 0);
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"[ERROR] Games folder not found!");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"Expected location: E:\\games");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"Please create this folder and add your games.");
        }
        if (hStatusBar) {
            SendMessageW(hStatusBar, SB_SETTEXTW, 0, (LPARAM)L"Error: Games folder not found");
        }
        return;
    }
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        if (hListBox) {
            SendMessageW(hListBox, LB_RESETCONTENT, 0, 0);
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"[ERROR] Cannot read games folder!");
        }
        if (hStatusBar) {
            SendMessageW(hStatusBar, SB_SETTEXTW, 0, (LPARAM)L"Error: Cannot read folder");
        }
        return;
    }
    ...
    do {
        if ((findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) &&
            ...
            folderCount++;
            wstring gamePath = gamesFolder + L"\\" + findData.cFileName;
    ...
    if (hListBox) {
        ...
        if (games.empty()) {
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"[No games found]");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"Add game folders to: E:\\games");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"Each game should be in its own folder with");
            SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"the game's .exe file inside.");
        } else {
    ...
    if (hStatusBar) {
        if (gameCount == 0) {
            wstring status = L"No games found in " + to_wstring(folderCount) + L" folders - Add games to E:\\games";
...
void FilterGames(const wstring& query) {
    if (!hListBox || games.empty()) return;
    ...
    if (matchCount == 0 && !lowerQuery.empty()) {
        SendMessageW(hListBox, LB_ADDSTRING, 0, (LPARAM)L"[No matches found]");
    }
...
void LaunchGame(int displayIndex) {
    if (games.empty() || displayIndex < 0) return;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            HWND hLabel = CreateWindowW(L"STATIC", L"Search:",
                WS_CHILD | WS_VISIBLE,
                10, 15, 60, 20,
                hWnd, NULL, NULL, NULL);
            SendMessageW(hLabel, WM_SETFONT, (WPARAM)hMainFont, TRUE);
