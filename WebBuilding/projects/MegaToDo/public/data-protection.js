...
function createDailyBackup() {
    ...
    if (lastBackup !== today) {
        ...
        localStorage.setItem('megaToDo_daily_' + today, JSON.stringify(backup));
        ...
        for (let i = 8; i < 30; i++) {
            ...
            const oldKey = 'megaToDo_daily_' + oldDate.toISOString().split('T')[0];
...
function checkDataIntegrity() {
    try {
        const backup = localStorage.getItem('megaToDo_backup');
        if (backup) {
