...
const NavPanel = () => {
  ...
  // Auto-collapse mobile sidebar on navigation
  useEffect(() => {
    if (isMobile) {
      setIsMobileExpanded(false);
    }
  ...
  // Shift main content on desktop by flagging the app container
  useEffect(() => {
    const appEl = document.querySelector(".app");
    if (!appEl) return;
    if (!isMobile && isDesktopExpanded) {
      appEl.dataset.navExpanded = "true";
    } else {
      delete appEl.dataset.navExpanded;
    }
    return () => {
      delete appEl.dataset.navExpanded;
    };
