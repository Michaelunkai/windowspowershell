...
const PerJobBarChart = ({
  ...
  const labelColor = isDark ? "#cbd5e0" : "#222222";
  const titleColor = isDark ? "#e2e8f0" : "#333333";
  const gridLineColor = isDark ? "#2d3748" : "#e6e6e6";
  const tooltipBg = isDark ? "#1e293b" : "#ffffff";
  const tooltipTextColor = isDark ? "#e2e8f0" : "#333333";
  const badgeBg = isDark ? "#1e3a5f" : "#eaf4fb";
  const badgeColor = isDark ? "#60a5fa" : "#2176bd";
  ...
  const options = {
    ...
    xAxis: {
      ...
      title: { text: null },
      labels: {
        style: { fontSize: "13px", whiteSpace: "nowrap", textOverflow: "ellipsis", color: labelColor, fontWeight: 500 },
        formatter: function () {
          return this.value;
        }
      }
    ...
    yAxis: {
      min: 0,
      title: { text: t ? t("charts.numberOfMatches") : "Number of Matches", align: "high", style: { color: titleColor } },
      ...
      formatter: function () {
        ...
        return `
          <div style="white-space: nowrap; min-width: 150px;">
            ...
            <span style="display:inline-block;margin-top:6px;padding:2px 8px;background:${badgeBg};color:${badgeColor};border-radius:4px;font-size:13px;font-weight:500;">${clickToViewLabel}</span>
          </div>
        `;
      }
    },
