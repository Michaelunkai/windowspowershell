...
const CompanyJobsPieChart = ({ data = [], onSliceClick, selectedCompany, companyColorMap = {}, t }) => {
  ...
  const labelColor = isDark ? "#cbd5e0" : "#333333";
  const tooltipBg = isDark ? "#1e293b" : "#ffffff";
  const tooltipTextColor = isDark ? "#e2e8f0" : "#333333";
  const borderColor = isDark ? "transparent" : "#fff";
  ...
  const options = {
    chart: {
      ...
      ...(isMobile ? { spacing: [10, 0, 0, 0], width: 390 } : { height: 400 })
    },
    title: {
      text: isMobile ? null : (t ? t("charts.topCompaniesByJobs") : "Top Companies by Number of Jobs"),
      style: { fontSize: "1rem", fontWeight: 500, color: titleColor },
      margin: 6
    },
    series: [{
    ...
    tooltip: {
      ...
      style: { fontSize: "13px", color: tooltipTextColor }
    },
    plotOptions: {
      pie: {
        allowPointSelect: true,
        cursor: "pointer",
        dataLabels: { enabled: !isMobile, style: { color: labelColor, textOutline: "none" } },
