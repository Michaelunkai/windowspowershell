      ...
      <div className="profile-flex-container" style={{ display: "flex", gap: "32px", flexWrap: "wrap" }} dir="auto">
        ...
        <>
          <div className="profile-container" style={{ flex: "1 1 340px", minWidth: 320, maxWidth: "max(500px, 35vw)" }}>
            ...
            {/* Persistent live regions — always in DOM so SR reliably picks up announcements */}
            <div className="profile-sr-only" role="status" aria-live="polite" aria-atomic="true">
              {toggleFeedback?.type === "success" ? toggleFeedback.message : ""}
            </div>
            <div className="profile-sr-only" role="alert" aria-live="assertive" aria-atomic="true">
              {toggleFeedback?.type === "error" ? toggleFeedback.message : ""}
            </div>
            {toggleFeedback && (
              <div
                className={`profile-toast profile-toast--${toggleFeedback.type}${toggleFeedback.isVisible ? " profile-toast--visible" : ""}`}
              >
                <span className="profile-toast-message" aria-hidden="true">{toggleFeedback.message}</span>
                <button
                  type="button"
