 ...
 *
 * Props:
 *   onNavigate    {fn}   called with { type, item } when a result is clicked
 *   inputRef      {ref}  optional external ref for the input element; allows parent
 *                        components (e.g. App.jsx) to programmatically focus the bar
 *                        via keyboard shortcuts (/ key via useKeyboardShortcuts hook).
 */
export default function SearchBar({ onNavigate, inputRef: externalInputRef }) {
  const [query, setQuery] = useState('')
  const [results, setResults] = useState(null) // null = not searched yet
  const [loading, setLoading] = useState(false)
  const [open, setOpen] = useState(false)
  const internalInputRef = useRef(null)
  // Prefer the externally-provided ref so App.jsx can call .focus() directly
